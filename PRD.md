...
| **Laugher (笑者)** | **仅走窗户 (Scene 2)** | **行动不规律**：有时在窗外远处站着不动，有时在窗外贴脸凝视不动，有时躲在窗后盲区（可能看不到）。<br>**攻击与伤害机制**：**只有在 Laugher 准备发动攻击时**（发出诡异笑声与敲窗声，贴在窗前狞笑），玩家才能用枪或刀攻击击杀它；**未攻击时**，玩家无法用刀伤害它，但在窗口开枪会将其吓退回草坪远处（不会逃离场景）。<br>**并发限制**：同一时间窗外最多仅有 1 只 Laugher。 | 听到笑声或敲玻璃声时立即切至窗口将其击杀；或在安全期于窗口鸣枪将其提前吓退回远处。 |
| **Mimic (拟态者)** | **仅爬梯子 (Scene 3)** | 沿梯子向上爬并敲击活板门。<br>**声音机制**：有自身独特嘶吼/咔哒声，有时会模仿 Walker 拆门声或 Laugher 笑声；**发出声音时必定在地图/画面可见**。<br>**视线对抗**：玩家切换至地窖直视它时，若其在底部会受惊**缩回黑暗**并暂停攀爬；若已爬至中段或更高则**继续上爬**。 | 听到可疑声音时切到地窖或查看监控核实；在它处于底部时用视线压制其上爬节奏并择机击杀。 |
...
## 10. AI 自动游玩与观战模式 (AI Autoplay Mode)

*   **定位**：内置战术 AI 全权接管防守决策、场景转场、装弹、压制与攻击，玩家无需操作即可观赏高水平防御过程，并可随时切入/退出托管。
*   **核心特性**：
    *   **多入口动态威胁评估**：按破窗/破板/顶门倒计时与容错机制状态，毫秒级量化计算正门、窗户、地窖的威胁指数。
    *   **武器与资源策略**：远距离优先霰弹枪必杀；空弹且贴脸时迅速切战术刀连击；安全期与波次休整期自动执行逐发压弹装填。
    *   **视线压制与拟态排查**：梯底 Mimic 窥探时执行视线威慑使其退回黑暗，发声显形时精准狙杀。
    *   **神经网络决策看板 (AI Thought Terminal)**：HUD 实时呈现 AI 思考推理、战术目标、行动状态与 3 路威胁雷达。
    *   **双端无缝开关**：支持从标题界面直接进入 AI 模式，或在游戏中通过 HUD 按钮随时开启/关闭 AI 托管。
